...
const GET = async ({ url }) => {
  try {
    ...
    const { data: nodes, error: nodesError } = await supabase.from("nodes").select("id").eq("topic_id", topicId);
    if (nodesError) {
      throw nodesError;
    }
...
const POST = async ({ request }) => {
  try {
    ...
    const { data: existingEdge } = await supabase.from("edges").select("id").eq("parent_node", parent_node).eq("child_node", child_node).eq("type", type).single();
    if (existingEdge) {
      return new Response(JSON.stringify({
