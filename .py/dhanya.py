...
# ---- Load custom font ----
font_path = r"C:\Users\Sreek\Downloads\helvetica-neue-55-cufonfonts\HelveticaNeue-Medium.otf"
helv_black = fm.FontProperties(fname=font_path)

# ---- Data ----
data = [
    ["Dysphonia","R49.0",0.4082804179,0.367,0.455,"<1e-300"],
    ["Voice and resonance disorders","R49*",0.4408890354,0.400,0.486,"<1e-300"],
    ["Other voice and resonance disorders","R49.8",0.6530447852,0.516,0.827,"3.980E-04"],
    ["Unspecified voice and resonance disorder","R49.9",0.7128996011,0.576,0.883,"1.923E-03"],
    ...
    ["Cognitive Communication Deficit","R41.841",1.318071729,1.198,1.450,"1.257E-08"],
    ["Unspecified symbolic dysfunctions","R48.9",1.555791861,1.136,2.132,"5.938E-03"],
    ["Other speech disturbances","R47.89",1.687107353,1.562,1.822,"<1e-300"],
    ["Dyslexia & other symbolic dysfunctions","R48*",1.698927631,1.500,1.925,"<1e-300"],
    ...
    ["Fluency disorder in conditions classified elsewhere","R47.82",6.523520796,5.269,8.076,"<1e-300"]
]

df = pd.DataFrame(data, columns=["Diagnosis","ICD","OR","CI_low","CI_high","p_value"])
...
for i, row in df.iterrows():
    if row["p_num"] > 0.05:  # not significant
        colors.append("grey")
    else:  # significant
        if row["OR"] > 1:
            colors.append("#2774AE")  # blue-ish
        else:
            colors.append("#FFD100")  # yellow-ish
...
plt.xlabel("Odds Ratio (95% CI)", fontproperties=helv_black)
plt.title("Forest Plot of Odds Ratios", fontproperties=helv_black)
